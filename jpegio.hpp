 ...
 * JPEG画像読み込み関数
 */
int readJpeg(std::string filename,       /* 画像ファイル名 */
	     BYTE **imageBuffer)   /* 画像データ先頭ポインタへのポインタ */
{
  ...
  char* filename_chars = new char[filename.size()+1];
  std::char_traits<char>::copy(filename_chars, filename.c_str(), filename.size()+1);

  /* ファイルを開く */
  if ((infile = fopen(filename_chars, "rb")) == NULL)
    {
 ...
 * JPEGヘッダ情報取得関数
 */
int readHeader(std::string filename,   /* 画像ファイル名 */
 ...
 * JPEG画像出力関数
 */
int writeJpeg(std::string filename,      /* ファイル名 */
	      ...
	      int quality)         /* JPEG出力画質(1-100) */
{
  ...
  /* ファイルを開く */
  if ((outfile = fopen(filename_chars, "wb")) == NULL)
    {
      fprintf(stderr, "can't open %s\n", filename_chars);
      return 0;
    }
